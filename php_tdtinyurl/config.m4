...
if test "$PHP_UQID" != "no"; then
  ...
  [
    PHP_ADD_LIBRARY(uqid,, TINYURL_SHARED_LIBADD)
...
if test "$PHP_HIREDIS" != "no"; then
  ...
  PHP_CHECK_LIBRARY($LIBNAME,$LIBSYMBOL,
  [
    PHP_ADD_LIBRARY(hiredis,, TINYURL_SHARED_LIBADD)
...
PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $UQID_DIR/lib64/, TINYURL_SHARED_LIBADD)
PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $HIREDIS_DIR/lib/, TINYURL_SHARED_LIBADD)
PHP_ADD_INCLUDE($HIREDIS_DIR/include)
PHP_ADD_INCLUDE($UQID_DIR/include)

PHP_SUBST(TINYURL_SHARED_LIBADD)
