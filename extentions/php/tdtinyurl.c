...
ZEND_BEGIN_ARG_INFO(arginfo_tdtinyurl_get_sqlite3_db_path, 0)
ZEND_END_ARG_INFO()
...
const zend_function_entry tdtinyurl_functions[] = {
	...
	PHP_FE(tdtinyurl_get_sqlite3_db_path,    	arginfo_tdtinyurl_get_sqlite3_db_path)
	PHP_FE_END
};
...
STD_PHP_INI_BOOLEAN("tdtinyurl.sqlite3_with_mode_enable",        "0", PHP_INI_ALL, OnUpdateBool,   sqlite3_with_mode_enable, zend_tdtinyurl_globals, tdtinyurl_globals)
STD_PHP_INI_ENTRY("tdtinyurl.sqlite3_db_path", "/usr/local/data/tdtinyurl/tdtinyurl.db", PHP_INI_ALL, OnUpdateString, sqlite3_db_path, zend_tdtinyurl_globals, tdtinyurl_globals)
...
static int tiny_url_sqlite3_connect(sqlite3 **pdb, const char *ppath) {
...
int tiny_url_sqlite3_close(sqlite3 *pdb) {
...
int tiny_url_sqlite3_quick_exec(const char *ppath, const char *psql) {

    sqlite3 *pdb;
    char *perrmsg = NULL;
    int errcode = 0;

    errcode = tiny_url_sqlite3_connect(&(pdb), ppath);
    if(SQLITE_OK != errcode) {
        return errcode;
    }

	errcode = sqlite3_exec(pdb, "BEGIN", NULL, NULL, &perrmsg);
    if(SQLITE_OK != errcode) {

        php_error_docref(NULL TSRMLS_CC, E_WARNING, "sqlite3 can not start transaction : %s", perrmsg);
        return errcode;
	}
    ...
    if(SQLITE_OK != errcode) {
        ...
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "sqlite3 exec error : %s", perrmsg);
    ...
    errcode = tiny_url_sqlite3_close(pdb);
    if(SQLITE_OK != errcode) {
        return errcode;
    }
...
int tiny_url_sqlite3_exec(sqlite3 *pdb, const char *psql) {
...
static int tiny_url_sqlite3_transaction(sqlite3 *pdb, int transact_type) {
	...
	switch(transact_type)  {
		...
		case TORDEN_TINYURL_SQL_BEGIN_TRANSACTION:
			errcode = sqlite3_exec(pdb, "BEGIN", NULL, NULL, &perrmsg);
			break;
		case TORDEN_TINYURL_SQL_COMMIT_TRANSACTION:
			errcode = sqlite3_exec(pdb, "COMMIT", NULL, NULL, &perrmsg);
			break;

		case TORDEN_TINYURL_SQL_ROLLBACK_TRANSACTION:
			errcode = sqlite3_exec(pdb, "ROLLBACK", NULL, NULL, &perrmsg);
			break;
		default:
			return -4;
			break;
	...
	if(SQLITE_OK != errcode) {
        ...
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "sqlite3 transation error : %s", perrmsg);
		sqlite3_free(perrmsg);
		sqlite3_close(pdb);
		return errcode;
	} else {
...
PHP_FUNCTION(tdtinyurl_set_url)
{
    ...
    //get uqid generating
    ptdtinyurl_key = (char *)safe_emalloc(sizeof(char), TDUQID_MALLOC_LEN, 1);
    seed_num = tduqid_get_str_to_number(purl);
    if(NULL == tduqid_get_str(ptdtinyurl_key, TDUQID_MAX_LEN, seed_num)) {
    ...
    switch(working_mode) {
        ...
        case PHP_TDTINYURL_LIVE:
			...
			//sqlite3
    		if(TINYURLG(sqlite3_with_mode_enable)) {
				snprintf(psqlbuf,500, TORDEN_TINYURL_SQL_INSET_TDTINYURL,ptdtinyurl_key, purl, expire_time_sec, action_mode, 0);
				errcode_sqlite3 = tiny_url_sqlite3_quick_exec(TINYURLG(sqlite3_db_path), psqlbuf);
				if(SQLITE_OK != errcode_sqlite3) {

					efree(ptdtinyurl_key);
                	php_error_docref(NULL TSRMLS_CC, E_ERROR, "can not insert tinyurl into sqlite3 : %s(%s)", TINYURLG(sqlite3_db_path), sqlite3_errstr(errcode_sqlite3));
 	               RETURN_FALSE;
				}
			}
            ...
            if(-1 == tiny_url_redis_connect(TINYURLG(redis_connect_mode), TINYURLG(redis_uds_sock_path), TINYURLG(redis_ip), atoi(TINYURLG(redis_port)))) {
                ...
                efree(ptdtinyurl_key);
                php_error_docref(NULL TSRMLS_CC, E_ERROR, "redis (%s:%s)connection fail", TINYURLG(redis_connect_mode), TINYURLG(redis_uds_sock_path));
                RETURN_FALSE;
            }
...
PHP_FUNCTION(tdtinyurl_get_sqlite3_db_path)
{
	...
	RETURN_STRING(TINYURLG(sqlite3_db_path),1);
}
